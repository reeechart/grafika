...
#define START_X_MINIMAP 0
#define START_Y_MINIMAP 0
#define MINIMAP_X_SIZE 1360
#define MINIMAP_Y_SIZE 760
#define START_X_MAP 0
#define START_Y_MAP 0
#define MAP_X_SIZE 1360
#define MAP_Y_SIZE 760
...
int sizeX = 50;
int sizeY = 50;
Canvas paintedWindow(Point(0, 0), Point(SCREEN_X_SIZE, SCREEN_Y_SIZE));
...
void moveOffset(int x, int y) {
    offsetX += x;
    if (offsetX >= MINIMAP_X_SIZE) offsetX = MINIMAP_X_SIZE - 1;
    if (offsetX < 0) offsetX = 0;
    offsetY += y;
    if (offsetY >= MINIMAP_Y_SIZE) offsetY = MINIMAP_Y_SIZE - 1;
